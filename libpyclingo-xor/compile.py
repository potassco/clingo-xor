...
cnt = []
with open(os.path.join(path, '..', 'libclingo-xor', 'clingo-xor.h')) as f:
    for line in f:
        if not re.match(r' *(#|//|extern *"C" *{|}$|$)', line):
            cnt.append(re.sub(r'[A-Z_]+_VISIBILITY_DEFAULT ', '', line).strip())
cnt.append('extern "Python" bool pyclingoxor_rewrite(clingo_ast_t *ast, void *data);')
code = '\n'.join(cnt)
...
ffibuilder.set_source("_clingoxor", """\
#include "clingo-xor.h"
""")

ffibuilder.emit_c_code('_clingoxor.c')
