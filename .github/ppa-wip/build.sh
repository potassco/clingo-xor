...
EOF
        ...
        build)
            ...
            (
                ...
                sed -i '/\.buildinfo$/d' ../clingo-xor_${VERSION}_source.changes
                debsign --no-re-sign -k744d959e10f5ad73f9cf17cc1d150536980033d5 ../clingo-xor_${VERSION}_source.changes
            )
            ;;
        put)
            VERSION="$(head -n 1 ${rep}/debian/changelog | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+\(-[a-z0-9]\+\)\?')"
            dput ppa:potassco/${ref} clingo-xor_${VERSION}_source.changes
            ;;
        clean)
            rm -rf \
                "${rep}"/app \
                ...
                "${rep}"/debian/clingo-xor.debhelper.log \
                "${rep}"/debian/clingo-xor.substvars \
                "${rep}"/debian/clingo-xor \
